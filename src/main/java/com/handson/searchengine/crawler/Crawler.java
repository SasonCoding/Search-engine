...
import com.handson.searchengine.config.AmazonSQSConfig;
...
public class Crawler {
    ...
    public void crawl(String crawlId, CrawlerRequest crawlerRequest) throws InterruptedException, IOException {
        ...
        String sqsMessage = om.writeValueAsString(CrawlerRecord.of(crawlId, crawlerRequest));
        queueMessagingTemplate.send(endPoint, MessageBuilder.withPayload(sqsMessage).build());
    }
    ...
    private void addUrlsToQueue(CrawlerRecord rec, List<String> urls, int distance) throws InterruptedException, JsonProcessingException {
        ...
        for (String url : urls) {
            if (!crawlHasVisited(rec, url)) {
                String sqsMessage = om.writeValueAsString(CrawlerRecord.of(rec).withUrl(url).withIncDistance());
                queueMessagingTemplate.send(endPoint, MessageBuilder.withPayload(sqsMessage).build());
            }
        }
    }
